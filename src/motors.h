...
public:
    ...
    void update_controllers(float velocity, float omega, float steering_adjustment) {
        ...
        float tangent_speed = m_omega * MOUSE_RADIUS * DEG_TO_RAD;
    ...
    void set_left_motor_pwm(int pwm) {
        ...
        if (pwm < 0) {
            fast_write_pin(MOTOR_LEFT_DIR1, LOW);
            fast_write_pin(MOTOR_LEFT_DIR2, HIGH);
            analogWrite(MOTOR_LEFT_PWM, -pwm);
        } else {
            fast_write_pin(MOTOR_LEFT_DIR1, HIGH);
            fast_write_pin(MOTOR_LEFT_DIR2, LOW);
            analogWrite(MOTOR_LEFT_PWM, pwm);
        }
    }
    // TODO: HARDWARE DEPENDENCY
    void set_right_motor_pwm(int pwm) {
        ...
        if (pwm < 0) {
            fast_write_pin(MOTOR_RIGHT_DIR1, LOW);
            fast_write_pin(MOTOR_RIGHT_DIR2, HIGH);
            analogWrite(MOTOR_RIGHT_PWM, -pwm);
        } else {
            fast_write_pin(MOTOR_RIGHT_DIR1, HIGH);
            fast_write_pin(MOTOR_RIGHT_DIR2, LOW);
            analogWrite(MOTOR_RIGHT_PWM, pwm);
        }
    }
